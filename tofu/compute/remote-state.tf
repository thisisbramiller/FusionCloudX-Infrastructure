...
# (homelab_network_id) + the FOUNDATION TEMPLATE IDs (ubuntu_template_vm_id 9001,
# debian_lxc_template_file_id), never live IPs. The unifi-host module accepts a
...
#
# Template outputs surfaced for the service calls:
#   - data.terraform_remote_state.network.outputs.ubuntu_template_vm_id      (9001)
#       -> passed as template_vm_id to every proxmox-vm service call.
#   - data.terraform_remote_state.network.outputs.debian_lxc_template_file_id
#       -> passed as template_file_id to the postgresql proxmox-lxc call.
# Cross-state clone-by-vm_id is safe because network/ (P3) applies BEFORE
# compute (P5), so 9001 + the vztmpl physically exist on the pve node first.
#

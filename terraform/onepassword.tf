...
# ------------------------------------------------------------------------------
# Backrest Credentials
# ------------------------------------------------------------------------------

# Backrest SSH key for pre-backup hooks (SSH into source VMs)
resource "onepassword_item" "backrest_ssh_key" {
  vault    = var.onepassword_vault_id
  category = "secure_note"
  title    = "Backrest SSH Key"
  tags     = ["terraform", "backrest", "ssh", "homelab"]

  note_value = <<-EOT
    SSH key pair generated by Terraform for Backrest pre-backup SSH hooks.

    Usage:
    - Deployed to /opt/backrest/ssh/backrest_ed25519 on the Backrest VM
    - Used by Backrest pre-backup hooks to SSH into source VMs for native dumps
  ...
  section {
    ...
    field {
      label = "private_key"
      type  = "CONCEALED"
      value = tls_private_key.backrest.private_key_openssh
    }
  ...
  section {
    ...
    field {
      label = "public_key"
      type  = "STRING"
      value = tls_private_key.backrest.public_key_openssh
    }

    field {
      label = "public_key_fingerprint_sha256"
      type  = "STRING"
      value = tls_private_key.backrest.public_key_fingerprint_sha256
    }
  }
